...
<div class="container  container-100k">
        ...
        <div class="row row_col row-cols-1 row-cols-sm-2 row-cols-md-4">
            ...
            <a class="btn btn-white" style="border: none;" href="{{route('chitiet',$value->id)}}">
                <div class="card" style="width: 18rem ; margin: 10px;height: 400px;">
                    <img style="max-height: 200px;" src="{{url('upload')}}/{{$value->image}}" class="card-img-top" width="90%" alt="...">
                    <div class="card-body">
                        <p class="card-text">{{$value->name}}</p>
    ...
    <div class="page-1">
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                <li class="page-item">
                    <a class="page-link {{$pro->previousPageUrl()}}" href="#" aria-label="Previous">
                        <span aria-hidden="true">&laquo;</span>
                    </a>
                    @for($i=1 ;$i<=$pro->total();$i++)


                <li class="page-item {{($i == $pro->currentPage()?'active': '' )}}"><a class="page-link" href="?page={{$i}}">{{$i}}</a></li>


                @endfor
                <li class="page-item">
                    <a class="page-link {{$pro->nextPageUrl()}}" href="#" aria-label="Next">
